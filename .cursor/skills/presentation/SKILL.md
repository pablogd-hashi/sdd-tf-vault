...
This workspace vendors the public package at `vendor/presentation_skill` (pinned in `vendor/SOURCE.md`). This file is the **only** root skill exposed to Cursor. Supporting Markdown in this directory is progressive disclosure, not additional root skills.

| Item | This workspace |
|------|----------------|
| Public package | `vendor/presentation_skill` |
| Root contract (canonical) | `vendor/presentation_skill/skills/skill_presentation.md` |
| CLI | `bash vendor/presentation_skill/scripts/presentation-skill` |
| Python env | `vendor/presentation_skill/.venv` (local, gitignored) |
| New decks | `decks/<deck-name>/` (gitignored) |
| Image generation | Cursor `GenerateImage` tool. Optional provider keys stay in workspace `.env`, never in the vendored tree. |

Before scaffolding or editing a deck, read the canonical root skill and the supporting files next to this overlay (`reference.md`, `reveal_decks.md`, `generated_assets.md`, `speaker_notes.md`). If this overlay and the vendored `skills/` files disagree, prefer the vendored copies.
...
**Out of scope:** PPTX editing, generic image prompting, design review checklists. Image API calls belong to this installing workspace (Cursor `GenerateImage`, or optional keys in `.env`), not the vendored package.
...
- If a distribution PDF is requested for an image deck, it is built with `vendor/presentation_skill/scripts/presentation-skill export-pdf` (compat gate passed, link count matches the overlay plan) — not by printing from a browser.
...
bash vendor/presentation_skill/scripts/presentation-skill "Topic" --mode reveal --assets mixed --output decks/topic
bash vendor/presentation_skill/scripts/presentation-skill "Topic" --request "HTML only, no image generation" --output decks/topic
```

**PDF export (image decks — the default way to produce the distribution PDF)**

```bash
bash vendor/presentation_skill/scripts/presentation-skill export-pdf decks/topic --check-only
bash vendor/presentation_skill/scripts/presentation-skill export-pdf decks/topic --output decks/topic/handout/slides.pdf
```
...
After init, read `decks/<deck-name>/README.md`. Active mode is at deck root; the other mode is under `examples/`.
...
**Image rendering** (inside scaffolded deck)

Prefer Cursor `GenerateImage` for full-slide images. The scaffold's `tools/generate_slides.py` is optional and needs provider keys from workspace `.env`, not from `vendor/presentation_skill`.
...
- Prefer Cursor `GenerateImage` in this workspace.
- Image decks that show URLs/QRs or embed live artifacts: add a clickable HTML overlay layer per [docs/clickable_overlays.md](../../../vendor/presentation_skill/docs/clickable_overlays.md) — prompt the zone, measure the rendered element's bbox by vision, inject padded `<a>`/iframe hotzones, verify with a draw-back check.

## Known traps

| Trap | How it shows up | What to do |
|------|-----------------|------------|
| Wrong install path | Agent reads a stale `.agents/skills/` or injected path that does not exist | Use this overlay (`.cursor/skills/presentation/SKILL.md`) and the vendored package at `vendor/presentation_skill`. Canonical upstream skill: `vendor/presentation_skill/skills/skill_presentation.md`. |
...
- Clickable links / live-artifact embeds on image slides: [docs/clickable_overlays.md](../../../vendor/presentation_skill/docs/clickable_overlays.md)
